...
const LOG_TYPE_META = {
    ...
    transfer_in:  { emoji: "💰", label: "Received from member"    },
...
async run(client, int, tools) {
    ...
    if (streakText) {
        embed.addFields({ name: "Streak Info", value: streakText, inline: false });
    }
    ...
    let creditLogField
    if (rawLogs.length === 0) {
        creditLogField = `Balance: ${tools.commafy(credits)}\n_No transactions recorded yet._`
    } else {
        const balanceLine = `Balance: ${tools.commafy(credits)}`
        ...
        const footerLine = `_Showing last ${rawLogs.length} transaction${rawLogs.length === 1 ? "" : "s"}._`

        creditLogField = `${balanceLine}\n${logLines.join("\n")}\n${footerLine}`
    }

    embed.addFields({
        name: `Credit History (last ${CREDIT_LOG_DISPLAY_COUNT})`,
